...
const Filter: FC<FilterProps> = ({
  ...
  return (
    <>
      ...
      <section className="hidden min-h-[784px]  border-[2px] border-mooduck-black p-[30px] xl:flex xl:w-[380px] xl:flex-col xl:gap-y-5 2xl:flex 2xl:w-[525px] 2xl:flex-col 2xl:gap-y-5">
        ...
        <div className="flex flex-col gap-y-[10px]">
          <MyButton
            ...
            onClick={() => {
              createResults()
              test({limit: 20, page: page, genre: resultGenres.length ? resultGenres?.join('-') : undefined, author: resultAuthors.length ? resultAuthors?.join('-') : undefined })
            }}
          >
            применить фильтры
          </MyButton>
